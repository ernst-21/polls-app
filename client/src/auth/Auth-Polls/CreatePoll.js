...
import { Form, Input, Button, Modal, Card, Grid } from 'antd';
...
const CreatePoll = () => {
  ...
  const { mutate: createMutation, isError } = useMutation(
    (poll) =>
      create(poll, { t: jwt.token })
        .then((res) => res.json())
        .then((data) => data),
    {
      onSuccess: (data) => {
  ...
  const info = () => {
    Modal.info({
      title: 'Polls will be submitted as shown in this preview.',
      icon: null,
      content: (
        <Poll
      ...
      >
        <Form form={form} name="dynamic_form_nest_item" onFinish={clickSubmit}>
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: 'Option #2 is required' },
              ({ getFieldValue }) => ({
                async validator(_, value) {
                  ...
                  if (
                    value &&
                    getFieldValue('question').trim().length > 0 &&
                    getFieldValue('first').trim().length > 0
                  ) {
                          ...
                          rules={[
                            {
                              required: true,
                              message:
                                'Type an option or close this field to validate the poll'
                            }
                          ]}
                        >
                          <Input />
          ...
          <p>
            Duplicated options will not be submitted. Please always validate and
          ...
          <Form.Item>
            <div className="create-poll_button-container">
              <Button
                className="create-poll_reset-btn"
                htmlType="button"
                onClick={onReset}
              >
                Reset
              ...
              <Button
                type="primary"
                htmlType="submit"
                disabled={!poll || !validateOptions || !submitReady}
              >
                Submit
              </Button>
            </div>
